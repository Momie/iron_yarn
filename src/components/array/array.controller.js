(function() {
    ...
    .controller('ArrayCtrl', function($scope, $rootScope, $timeout, $document, $state, $stateParams, $localStorage, $window, project, panel, lodash, alerts, msgHandler) {
        var _holder,
            _holderSvg,
            _canvasWidth = 928,
            _canvasHeight = 464,
        ...
        var _drawSmallPanel = function(){
          ...
          for(var count = 0; count < _panels.length; count++){
            x = ($localStorage.ironridge[project.info.id][_panels[count].data._id].x + _canvasWidth * 2) * _realSmallScale;
            y = ($localStorage.ironridge[project.info.id][_panels[count].data._id].y +  _canvasHeight * 2) * _realSmallScale;
        ...
        var _updatePannels = function(notUpdate){
          ...
          while(count < _panels.length) {
            obj = lodash.find(project.info.arrays.objects, fn);
        ...
        var _moveWithKeyboard = function (varToUpdate, isAugment){
          ...
          for (var i = 0; i < $rootScope.selectedPanels.length; i++) {
            ...
            if((varToUpdate === 'x' && $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] + dep > (limitXMin + localObj.getCellWidth()+1) && $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] + dep < (limitXMax - widthPanel)) || (varToUpdate === 'y' && $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] + dep > (limitYMin + localObj.getCellHeight()+3) && $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] + dep < (limitYMax - heightPanel))){
              $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] += dep;
              localObj.move($localStorage.ironridge[project.info.id][localObj.data._id].x, $localStorage.ironridge[project.info.id][localObj.data._id].y);
            }
            else {
              if(varToUpdate === 'x' && $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] + dep <= (limitXMin + localObj.getCellWidth()+1)){
                $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] = (limitXMin + localObj.getCellWidth()+1);
              }
              else if(varToUpdate === 'x' && $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] + dep >= (limitXMax - widthPanel)){
                $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] = limitXMax - widthPanel;
              }
              else if (varToUpdate === 'y' && $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] + dep <= (limitYMin + localObj.getCellHeight()+3)) {
                $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] = (limitYMin + localObj.getCellHeight()+3);
              }
              else if(varToUpdate === 'y' && $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] + dep >= (limitYMax - heightPanel)){
                $localStorage.ironridge[project.info.id][localObj.data._id][varToUpdate] = limitYMax - heightPanel;
              }
            }
          }
        };
        ...
        var _updateOrientationStatus = function(){
          ...
          if(($rootScope.switchEngineeringOrientation) && ($rootScope.nbrPanelsPortrait || (!$rootScope.nbrPanelsPortrait && !$rootScope.nbrPanelsLandscape))){
        ...
        var _draw = function(obj) {
            var pppanel = panel.instance(_holder);
        ...
        var _init = function() {
          var ob;
          _setStage();
          project.info.arrays.objects.forEach(function(obj) {
            obj.cells = lodash.toArray(obj.cells);
          });

          _panels = [];
          ob = project.info.arrays.objects;
        ...
        $scope.onSelectorClick = function($event) {
          ...
          if(!$scope.disableArrayBtn){
            if ($rootScope.project.modules && project.info.modules.model && project.info.modules.model.id)
        ...
        $scope.onDeleteClick = function($event) {
          ...
          if(!$scope.disableDeleteBtn){
            $rootScope.isSaved = false;
            _panelApply('destroy');
            project.info.arrays.objects = project.info.arrays.objects.filter(Boolean);
            //project.updateArrays();
            _updatePannels(true);
            _drawSmallPanel();
          }
        ...
        $scope.addPanel = function(row, col, $event) {
          ...
          project.info.arrays.objects.push(panelObj);
          project.info.arrays.count++;
          ...
          panelObj.index = project.info.arrays.objects.length - 1;
          panel = _draw(panelObj);
          //project.updateArrays();
          panel.update();
          panel.save();
          panel.selectEffect();
        ...
        $scope.centerPanel = function($event, id){
          ...
          x = $localStorage.ironridge[project.info.id][id].x;
          y = $localStorage.ironridge[project.info.id][id].y;
        ...
        $scope.sowPreviewItem = function($event){
          ...
          if(project.info.arrays.objects.length < 13) return;
        ...
        $scope.sowNextItem = function($event){
          ...
          if(project.info.arrays.objects.length < 13) return;
          if($scope.pannelIndex < project.info.arrays.objects.length - 12) $scope.pannelIndex++;
        };
        ...
        $rootScope.showAlertOrientation = function(isPortrait){
            if(isPortrait && !$localStorage.ironridge[project.info.id].isAlerted){
              $localStorage.ironridge[project.info.id].isAlerted = true;
              msgHandler.handle('You have just selected landscape orientation for your array. </br> Make sure this installation method does not void your PV modules’ warranty.', null, 'warning', null, true, null);
            }
        };
        ...
        $rootScope.$on('project.arrays.updated', function() {
            ...
            if(project.info.arrays.objects.length){
              $scope.enableArraySaveBtn();
              $rootScope.disableDeleteOrientBtn();
            }
            else{
        ...
        $rootScope.$on('project.arrays.updated.error', function() {
          $scope.selectedPanel = null;
